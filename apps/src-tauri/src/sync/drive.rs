...
pub fn auth_start(state: &mut DriveState) -> Result<AuthStart> {
  ...
  let csrf_secret_for_thread = csrf_secret.clone();
  let (tx, rx) = oneshot::channel();
  std::thread::spawn(move || {
    if let Ok((mut stream, _)) = listener.accept() {
      let mut buffer = [0u8; 4096];
      if stream.read(&mut buffer).is_ok() {
        if let Some(code) = parse_code_from_request(&buffer, &csrf_secret_for_thread) {
          let _ = tx.send(code);
  ...
  state.csrf_state = Some(csrf_secret);
...
pub async fn drive_sync(db_mutex: &std::sync::Mutex<Database>) -> Result<()> {
  ...
  let _local_metadata_path = write_local_metadata(&local_metadata)?;
